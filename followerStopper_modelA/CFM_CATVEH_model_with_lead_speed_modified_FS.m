%% 
function dx = CFM_CATVEH_model_with_lead_speed_modified_FS(t,x,uMin,uMax,params,exp_num)
...
if(~isfield(params,'external_r'))
    u = dyn_modified_follower_stopper(d_rel,v_rel,v_f,uMin,uMax);
%     [dx_vehicle,~] = dyn_modified_follower_stopper(t,x_vehicle,d_rel,v_rel,uMin,uMax,delay_size,exp_num);
else
    if(isempty(params.external_r))
        u = dyn_modified_follower_stopper(d_rel,v_rel,v_f,uMin,uMax,v_lead);
%         [dx_vehicle,~] = dyn_modified_follower_stopper(t,x_vehicle,d_rel,v_rel,uMin,uMax,delay_size,exp_num,v_lead);
    else
        u = dyn_modified_follower_stopper(d_rel,v_rel,v_f,uMin,uMax,params.external_r);
%          [dx_vehicle,~] = dyn_modified_follower_stopper(t,x_vehicle,d_rel,v_rel,uMin,uMax,delay_size,exp_num,params.external_r);
    end
end
