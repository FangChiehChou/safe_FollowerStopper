...
v_cmd_FS = zeros(size(d_rel,1),size(d_rel,2));

v_cmd_mFS = zeros(size(d_rel,1),size(d_rel,2));

r = 30;
v_AV = 30;

for m = 1:1:size(d_rel,1)
   
    for n = 1:1:size(d_rel,2)
      
        v_cmd_FS(m,n) = follower_stopper(d_rel(m,n),v_rel(m),v_AV,r);   %get speed command of the FS given the state
        
        v_cmd_mFS(m,n) = modified_FollowerStopper(d_rel(m,n),v_rel(m),v_AV,r);   %get speed command of the FS given the state
...
plot(t_s,v_cmd_mFS(1,:))
hold on
plot(t_s,v_cmd_mFS(2,:))
plot(t_s,v_cmd_mFS(3,:))
plot(t_s,v_cmd_mFS(4,:))

set(gca,'FontSize',30)
xlabel('Time[s]','FontSize',30)
ylabel('Speed command[m]','FontSize',30)
...
for m = 1:1:size(d_rel,1)
    ...
    for n = 1:1:size(d_rel,2)
        ...
        v_cmd_mFS(m,n) = modified_FollowerStopper(d_rel(m,n),v_rel(m),v_AV(m),r);   %get speed command of the FS given the state
