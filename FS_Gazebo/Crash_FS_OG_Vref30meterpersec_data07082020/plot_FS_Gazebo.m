...
plot(toyota_vel.time,toyota_vel.data,'LineWidth',2);
xlabel('Time[s]','FontSize',40)
ylabel('Speed[m/s]','FontSize',40)
set(gca,'FontSize',40)
legend('leader_{spd}','follower_{spd}','gazebo-lead_{speed}')
%% figures for paper
...
hold on
plot(data_time,test_data_v_f,'-.','LineWidth',2)
plot((cat_vel.time-cat_vel.time(1)),cat_vel.data,'-.','LineWidth',2,'Color',[0.8500, 0.3250, 0.0980])
...
set(gcf, 'Color', 'w');
% xlim([0 20])
eval(['export_fig ','Sim_newFS_Gazebo_LS',' -pdf']);
