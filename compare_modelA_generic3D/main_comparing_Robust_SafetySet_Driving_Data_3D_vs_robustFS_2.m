...
%%
%% safet set for original FS with timeHeadway verification

load('value_function_originalFS_timeHeadway_verification.mat')
...
figure()
[XX,YY,ZZ] = meshgrid(timeheadway_safetySet_origin_FS.x_new_grid,timeheadway_safetySet_origin_FS.y_new_grid,timeheadway_safetySet_origin_FS.z_new_grid);
V = permute(-timeheadway_safetySet_origin_FS.new_val_func,[2 1 3]);
P = patch(isosurface(XX,YY,ZZ,V,0));
isonormals(XX,YY,ZZ,V,P)
P.FaceColor = 'red';
P.EdgeColor = 'none';
...
lighting gouraud
xlabel('Distance[m]','FontSize',20);
ylabel('Leader speed[m/s]','FontSize',20);
zlabel('Follower speed[m/s]','FontSize',20)
set(gca,'FontSize',20)
grid on


[timeHeadwaySafetySet_originalFS] = absolute2relative(timeheadway_safetySet_origin_FS.x_new_grid,timeheadway_safetySet_origin_FS.y_new_grid,timeheadway_safetySet_origin_FS.z_new_grid,-timeheadway_safetySet_origin_FS.new_val_func);

x_grid_robust3D = timeHeadwaySafetySet_originalFS.x_new_grid;
y_grid_robust3D = timeHeadwaySafetySet_originalFS.y_new_grid;
z_grid_robust3D = timeHeadwaySafetySet_originalFS.z_new_grid;
val_func_timeHeadwaySafetySet_originalFS = timeHeadwaySafetySet_originalFS.new_val_func;

%% plot value function 3D Generic in the new coordinate
[XX,YY,ZZ] = meshgrid(x_grid_robust3D,y_grid_robust3D,z_grid_robust3D);
V = permute(val_func_timeHeadwaySafetySet_originalFS,[2 1 3]);
...
safetySlice_generic3D = val_func_timeHeadwaySafetySet_originalFS(:,:,1);
...
for frame_index = 1:1:length(v_follower_g)
    ...
    safetySlice_generic3D = val_func_timeHeadwaySafetySet_originalFS(:,:,frame_index);
