...
[XX,YY] = meshgrid(d_rel,v_rel);

vec_x = zeros(size(XX,1),size(XX,2));  %\dot{v_{rel}}
vec_y = zeros(size(XX,1),size(XX,2));  %\dot{d_{rel}}
...
for i = 1:1:size(XX,1)
    for j = 1:1:size(XX,2)
        d_rel_temp = XX(i,j);
        v_rel_temp = YY(i,j);
        a_av = dyn_modified_follower_stopper(d_rel_temp,v_rel_temp,vehicle_speed,uMin,uMax,r);
...
for i =1:1:length(d_rel_set)
    ...
    plot(v_lead-y(:,2),y(:,1),'Color',[0.4660 0.6740 0.1880],'LineWidth',2);
